...
class dmSocialUserFactory
{
  ...
  {
    // maybe later :)
    //$user_class = sfConfig::get('app_social_user_class', 'DmUser');
  ...
  public function isCompatible($user)
  {
    ...
    foreach($this->getKeys() as $key)
    {
      ...
      if(is_callable(array($user, $method)))
      {
        if($user->$method() != $social_user->$method())
        {
          $compatible = false;
          break;
        }
      ...
      {
        throw new dmException(sprintf('"%s" don\'t have field "%s"', get_class($user, $key)));
      }
    }
