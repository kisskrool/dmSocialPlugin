 ...
 * @since 21 août 2010
 */
class dmOAuth1 extends dmOAuth
{
  /**
   *
  ...
  /**
   * (non-PHPdoc)
   * @see plugins/dmDoctrineOAuthPlugin/lib/dmOAuth::requestAuth()
   */
  ...
  /**
   * (non-PHPdoc)
   * @see plugins/dmDoctrineOAuthPlugin/lib/dmOAuth::getAccessToken()
   */
  public function getAccessToken($verifier, $parameters = array())
  {
    ...
    $token->setTokenSecret($oauth_token_secret);
    $token->setName($this->getName());
    $token->setStatus(DmSocialToken::STATUS_ACCESS);
    $token->setOAuthVersion($this->getVersion());
